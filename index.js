...
app.post('/api/cv-builder/create', async (req, res) => {
  try {
    ...
    if (!createRes.ok) {
      ...
      return res.status(500).json({ error: 'Failed to create resume', details: err });
    }
      ...
      sections: {
        ...
        education: {
          ...
          items: (education || []).map((edu, i) => ({
            id: `edu-${i}`, visible: true,
            institution: edu.institution || '', studyType: edu.degree || '',
            area: '', startDate: edu.startDate || '', endDate: edu.endDate || '',
            summary: '', url: { label: '', href: '' }, score: '',
          })),
        },
        skills: {
          name: 'Skills', columns: 1, separateLinks: true, visible: true, id: 'skills',
          items: (skills || []).map((skill, i) => ({
            id: `skill-${i}`, visible: true, name: skill, description: '', level: 3, keywords: [],
          })),
        },
      },
    };
    ...
    const patchRes = await fetch(`${RXRESUME_BASE}/resume/${resumeId}`, {
      ...
      body: JSON.stringify({
        metadata: {
          ...
          typography: { font: { family: 'Inter', subset: 'latin', variants: ['regular','600'], size: 14 }, lineHeight: 1.5, hideIcons: false, underlineLinks: true },
          page: { margin: 18, format: 'a4', options: { breakLine: true, pageNumbers: true } },
          theme: { background: '#ffffff', text: '#000000', primary: '#1a3c2e' },
          locale: 'en', date: { format: 'MMMM yyyy' }, notes: '',
        },
        data: resumeData,
      }),
    });
    ...
    if (!patchRes.ok) {
      ...
      return res.status(500).json({ error: 'Failed to update resume', details: err });
    }

    res.json({
      resumeId,
      slug: resume.slug,
      viewUrl: `https://rxresu.me/resume/${resume.slug}`,
      printUrl: `https://rxresu.me/api/resume/${resumeId}/print`,
    });
  } catch (err) {
